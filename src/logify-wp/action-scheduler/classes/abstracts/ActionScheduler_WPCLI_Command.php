...
abstract class ActionScheduler_WPCLI_Command extends \WP_CLI_Command {
	...
	public function __construct( array $args, array $assoc_args ) {
		if ( ! defined( 'WP_CLI' ) || ! constant( 'WP_CLI' ) ) {
			...
			throw new \Exception( sprintf( __('The %s class can only be run within WP CLI.', 'logify-wp' ), get_class( $this ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- Exception is only thrown in CLI context, not displayed to users
		}
