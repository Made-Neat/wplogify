	...
	public function execute() {
		...
		$progress_bar = \WP_CLI\Utils\make_progress_bar(
			sprintf(
				...
				_n('Deleting %d action', 'Deleting %d actions', $this->action_counts['total'], 'logify-wp' ),
		...
		$format = _n('Deleted %1$d action', 'Deleted %1$d actions', $this->action_counts['deleted'], 'logify-wp' ) . ', ';
		/* translators: %2$d: number of actions deletions failed */
		$format .= _n('%2$d failure.', '%2$d failures.', $this->action_counts['failed'], 'logify-wp' );
