...
class Error_Tracker
{
    private static $start_time; // Start time for the error capturing process
     ...
     */
    public static function schedule_background_task($start_time)
    {
        // Unschedule previous actions to avoid duplication
        as_unschedule_action('logify_wp_process_errors');

        error_log("Strat Background:".get_option('logify_wp_capture_start_time'));

        // Schedule a recurring action every 10 seconds to process errors
        as_schedule_recurring_action(time(), 10, 'logify_wp_process_errors', [], 'logify-wp');
    }
    ...
    public static function stop_all_scheduled_actions()
    {
        ...
        as_unschedule_all_actions('logify_wp_process_errors');
        error_log("END Period: ");
    }
    ...
    public static function process_new_errors()
    {
        ...
        error_log("Capture Time131: " . time() . "       " . get_option('logify_wp_capture_start_time'));
        // error_log(print_r(as_get_scheduled_actions('wp'), true));
        // Check if the capture period has passed, then stop the scheduled task
        if (self::$errors_capture_period == "10mins" && (time() - get_option('logify_wp_capture_start_time')) > 10 * 60) {
            self::stop_all_scheduled_actions();
            return;
        } elseif (self::$errors_capture_period == "30mins" && (time() - get_option('logify_wp_capture_start_time')) > 30 * 60) {
            self::stop_all_scheduled_actions();
            return;
        } elseif (self::$errors_capture_period == "1hour" && (time() - get_option('logify_wp_capture_start_time')) > 60 * 60) {
            self::stop_all_scheduled_actions();
            return;
        }
    }
